...
2. **SQL Editor**에서 아래 순서로 실행 (한 번만)
   - `supabase/schema.sql` — 기본 테이블·채팅
   - `supabase/migration-business-reports.sql` — 대시보드·리포트용 테이블
   - `supabase/migration-rerun-safe.sql` — RLS 정책 + 식단 사진 Storage + Realtime  
     (이미 business 마이그레이션을 돌렸다가 policy 오류가 났다면 **이 파일만** 실행해도 됨)
   - `migration-daily-report-realtime.sql`은 **rerun-safe를 실행했다면 생략** (내용 중복)
3. **Authentication** 설정 (Email은 기본 활성화 — 별도 켤 필요 없음)
   - **Sign In / Providers** → **Email** 섹션 확인
   - **Allow new users to sign up** 켜기
   - 로컬 테스트 시: **Confirm email** 끄기 (가입 후 바로 로그인 가능)
   - **URL Configuration** → Site URL: `http://localhost:3000`, Redirect URLs에 `http://localhost:3000/**` 추가
